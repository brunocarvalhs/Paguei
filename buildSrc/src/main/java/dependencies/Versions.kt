package dependencies

object Versions {
    object Core {
        const val MULTIDEX = "2.0.1"
        const val KTX = "1.9.0"
        const val LIFECYCLE_RUNTIME = "2.6.1"
    }

    object UI {
        const val MATERIAL = "1.9.0"
        const val FRAGMENT_KTX = "1.6.0"
        const val LIFECYCLE_VIEWMODEL_KTX = "2.6.1"
        const val APPCOMPAT = "1.6.1"
        const val DATABINDING_COMMON = "8.0.2"
        const val DATABINDING_COMPILER = "8.0.2"
        const val CONSTRAINT_LAYOUT = "2.1.4"
        const val COORDINATOR_LAYOUT = "1.2.0"
        const val NAVIGATION_FRAGMENT = "2.6.0"
        const val NAVIGATION_UI = "2.6.0"
    }

    object Test {
        const val JUNIT = "4.13.2"
        const val JUNIT_ANDROID = "1.1.4"
        const val ESPRESSO_CORE = "3.5.0"
        const val MOCKITO = "4.11.0"
    }

    object Navigation {
        const val NAVIGATION_RUNTIME = "2.6.0"
    }

    object Hilt {
        const val ANDROID = "2.46"
        const val ANDROID_COMPILER = "2.46"
        const val ANDROID_TESTING = "2.46"
    }

    object Firebase {
        const val BOM = "32.2.0"
        const val CRASHLYTICS = ""
        const val ANALYTICS = ""
        const val AUTH = "22.1.0"
        const val FIRESTORE = "24.7.0"
        const val PLAY_SERVICES_AUTH = "20.6.0"
        const val COROUTINES_PLAY_SERVICES = "1.7.2"
        const val UI_AUTH = "8.0.2"
    }

    object Network {
        const val GSON = "2.10.1"
        const val RETROFIT = "2.9.0"
        const val RETROFIT_GSON = "2.9.0"
        const val OKHTTP = "4.9.0"
        const val OKHTTP_LOGGING = "4.9.0"
    }

    object Glide {
        const val GLIDE = "4.11.0"
        const val GLIDE_COMPILER = "4.11.0"
    }
}